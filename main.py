...
def main():
    ...
    zhat = (0, 0, 1)
    cameras = {
        'A': Camera('A', width, height, fov_h, fov_v,
            Quaternion(axis=xhat, degrees=-60)*Quaternion(axis=zhat, degrees=-90)),
        'B': Camera('B', width, height, fov_h, fov_v,
            Quaternion(axis=xhat, degrees=60)*Quaternion(axis=zhat, degrees=-90)),
        'C': Camera('C', width, height, fov_h, fov_v,
            Quaternion(axis=zhat, degrees=180+53))
    }
...
class Camera:
    def __init__(self, name, width, height, fov_h, fov_v, q_body2cam):
        ...
        self.gnomonic_scale = (((self.width - 1)/2)/tan(fov_h/2) +
                               ((self.height - 1)/2)/tan(fov_v/2))/2
        ...
        # Average horizontal and vertical scaling to ensure square pixels
        self.stereographic_scale = (((self.width - 1)/2)/(2*tan(fov_h/4)) +
                                    ((self.height - 1)/2)/(2*tan(fov_v/4)))/2
    ...
    def gn_to_px(self, x, y):
        return (int(round(self.gnomonic_scale*x + (self.width - 1)/2)),
                int(round(self.gnomonic_scale*y + (self.height - 1)/2)))

    def st_to_px(self, x, y):
        return (int(round(self.stereographic_scale*x + (self.width - 1)/2)),
...
class Spacecraft:
    ...
    def advance(self, dt):
        ...
        q_body_t02tf = (Quaternion(axis=self.omega_body, angle=dt*norm(self.omega_body))
                        if norm(self.omega_body) > 0
                        else Quaternion())
        # Only advance attitude, not position
        return Spacecraft(self.position_world, self.velocity_world,
                          self.q_world2body*q_body_t02tf, self.omega_body)

    def as_dict(self):
        return {'position_world': self.position_world.tolist(),
...
class ObservedBody:
    def __init__(self, body, spacecraft):
        ...
        self.direction_body = spacecraft.q_world2body.conjugate.rotate(self.direction_world)
...
class Detection:
    ...
    def __init__(self, obs_body, camera):
        ...
        self.center_st = (self.rho_center/rho)*self.direction_st if rho > 0 else self.direction_st

    def is_in_frame(self):
        if self.direction_cam[2] < 0: return False
        # FIXME: Choose point in interior closest to center (may not be on edge)
        if self.rho_center == 0: return True
        p = ((self.rho_center - self.radius_st)/self.rho_center)*self.center_st
        return (p[0] > self.camera.x_st[0] and
                p[0] < self.camera.x_st[-1] and
                p[1] > self.camera.y_st[0] and
                p[1] < self.camera.y_st[-1])

    def as_dict(self):
        return {'body': self.obs_body.body.name,
    ...
    for camera in cameras.values():
        ...
        for f in range(n_frames):
            ...
            if detection_dicts:
                ...
                img = render_bodies(camera, spacecraft_f, obs_f, colors_bgr, illuminator)
                if (camera.name == 'A'):
                    name = '1'
                elif (camera.name == 'B'):
                    name = '2'
                ...
                frame_dict = {'time': tf,
                              ...
                              'omega_cam': camera.q_body2cam.conjugate.rotate(spacecraft.omega_body).tolist(),
...
def render_bodies(camera, spacecraft, obs_bodies, colors_bgr, illuminator):
    ...
    bsins = [np.asarray(math.sin(b.angular_radius)**2, dtype=camera.x_gn.dtype) for b in obs_bodies]
    ...
    for row in range(camera.height):
        ...
        tr = row*line_time
        spacecraft_r = spacecraft.advance(tr)  # Alternative: increment spacecraft by line_time
        ...
        for b in range(len(obs_bodies)):
            ...
            if illuminator is None or obs_bodies[b] is illuminator:
                ...
                sun_pos_cam = q_world2cam.conjugate.rotate(
                        illuminator.relpos_world).astype(sc.dtype)
                body_pos_cam = q_world2cam.conjugate.rotate(
                        obs_bodies[b].relpos_world).astype(sc.dtype)
                radius = np.asarray(obs_bodies[b].body.radius, dtype=sc.dtype)
                img[row,...] += colors_bgr[b]*is_illuminated(sun_pos_cam, body_pos_cam, radius,
                                                             sc)[...,newaxis]
    return img
...
def render_stereographic(camera, obs_bodies, colors_bgr, illuminator):
    ...
    for b in range(len(obs_bodies)):
        ...
        if illuminator is None or obs_bodies[b] is illuminator:
            # Angular distance from each pixel ray to target center
            d = sin2_vangle(sc, direction_cam.astype(sc.dtype))
            img += colors_bgr[b]*(d < np.asarray(math.sin(obs_bodies[b].angular_radius)**2,
                                                 dtype=d.dtype))[...,newaxis]
        else:
            sun_pos_cam = camera.q_body2cam.conjugate.rotate(
                    illuminator.position_body).astype(sc.dtype)
            body_pos_cam = camera.q_body2cam.conjugate.rotate(
                    obs_bodies[b].position_body).astype(sc.dtype)
            radius = np.asarray(obs_bodies[b].body.radius, dtype=sc.dtype)
            img += colors_bgr[b]*is_illuminated(sun_pos_cam, body_pos_cam, radius, sc)[...,newaxis]
    return img
...
def test_main():
    ...
    targets = [Body('TC', 1, np.array([0, 0, 4])),
               Body('TA', 1, np.array([0, 4, 4])),
               Body('TB', 1, np.array([0, -4, 4]))]
    spacecraft = Spacecraft(np.array([0, 0, 0]), np.array([0, 0, 0]), Quaternion(), omega_body)
