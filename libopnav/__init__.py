from numpy import arctan, arctan2, asarray, cross, dot, newaxis, reciprocal, sqrt, stack, tan
...
def stereographic_inv(x, y):
    ...
    return stack((-4*x, -4*y, (8 - norm)), axis=-1)*reciprocal(
            norm.astype(np.common_type(norm)))[...,newaxis]
