...
from numpy import arctan, array, sqrt, tan, pi
...
class TestLibopnav(unittest.TestCase):
    ...
    def test_stereographic_inv_array(self):
        x = array([0, 1/sqrt(3), 0])
        y = array([0, 0, 1/sqrt(3)])
        expected = array([[0, 0, 1], [-12/(13*sqrt(3)), 0, 11/13], [0, -12/(13*sqrt(3)), 11/13]])
    ...
    def test_stereographic_array(self):
        s = array([[0, 0, 1], [-12/(13*sqrt(3)), 0, 11/13], [0, -12/(13*sqrt(3)), 11/13]])
        x, y = stereographic(s)
    ...
    def test_rotate_frame(self):
        xyz = array([[0, 0, 1], [-0.5, 0, sqrt(3)/2], [0, -0.5, sqrt(3)/2], [-0.40824829, -0.40824829,  0.81649658]])
        q = Quaternion(axis=[0, 0, 1], angle=pi/2)
        r = rotate_frame(xyz, q)
        expected = array([[0, 0, 1], [0, -0.5, sqrt(3)/2], [0.5, 0, sqrt(3)/2], [0.40824829, -0.40824829,  0.81649658]])
